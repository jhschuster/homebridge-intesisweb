...
function createIntesisWebDevice({Service, Characteristic}) {
    ...
    /** Reports the cached physical power state used by effective setting switches. */
    IntesisWebDevice.prototype.isPhysicallyPowered = function () {
	const services = this.details && this.details.services;
	return Boolean(services && services.power && services.power.value === 1);
    };
	...
	for (const axisName of ["horizontalVanes", "verticalVanes"]) {
	    if (services[axisName]) this.updateAxisSwingState(axisName, services[axisName].value);
	}
    };
	...
	if (this.autoFanSpeedService) {
	    this.autoFanSpeedService.updateCharacteristic(
		Characteristic.On,
		this.isPhysicallyPowered() && rawValue === 0);
	}
    };
	...
	// A vane setting can remain enabled in AC Cloud while the unit is off. The
	// HomeKit Switch represents effective operation so it cannot make the whole
	// accessory tile claim that an idle mini-split is powered on.
	if (axisService) {
	    axisService.updateCharacteristic(
		Characteristic.On,
		this.isPhysicallyPowered() && rawValue === 10);
	}
