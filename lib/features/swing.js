...
function setupAxisSwing(device, axisName, service, userID, deviceID) {
	...
	.on("get", callback => {
	    ...
	    axis
		? callback(undefined, device.isPhysicallyPowered() && axis.value === 10)
	...
	.on("set", (value, callback) => {
		...
		device.platform.setValue(userID, deviceID, serviceID, intesisValue, err => {
		    ...
		    finish(err);
		    // HAP may optimistically publish the requested switch value after
		    // callback completion, so restore the effective power-aware state.
		    setImmediate(() => {
			const axis = device.details.services && device.details.services[axisName];
			if (axis) device.updateAxisSwingState(axisName, axis.value);
		    });
		});
	    }, callback);
	})
	.updateValue(device.isPhysicallyPowered() && service.value === 10);
}
