...
function setupFanSpeed(device, service, userID, deviceID) {
	...
	.on("get", callback => {
	    const rawValue = device.details.services && device.details.services.fanSpeed.value;
	    callback(undefined, rawValue === 0);
	})
	.on("set", (value, callback) => {
	    device.log.debug(`${device.name}: automatic fan speed SET`, value);
	    device.enqueueMutation(
		finish => device.setAutomaticFanSpeed(value, userID, deviceID, serviceID, finish),
		callback);
	});
